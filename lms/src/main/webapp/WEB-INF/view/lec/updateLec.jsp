						...
						<td>
							<select name="lectureRoomName" id="lectureRoom" class="form-control">
								...
								<c:forEach var="l" items="${lectureRoomList}">
									<option value="${l.lectureRoomName}" maxPerson="${l.lectureRoomNumber}" <c:if test="${l.lectureRoomName eq lectureInfo.lectureRoomName}">selected="selected"</c:if>>${l.lectureRoomName}</option>								
								</c:forEach>
							</select>
	   	...
	   	$('#btn').click(function(){
   			// 카테고리
			    ...
			    var endDate = endDateSplit[0] + endDateSplit[1] + endDateSplit[2];
		   		...
		   		} else if (!result && $('#beginClassError').text()) { // ex) 12월 50일
			    	$('#beginClassError').text('개강일의 날짜형식이 유효하지 않습니다.');
			    } else if (today > beginDate) { // 개강일은 오늘 날짜 이상
